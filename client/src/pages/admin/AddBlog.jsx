    ...
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
        ...
        <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
          ...
          <button
            disabled={loading}
            className={`absolute bottom-1 right-2 ml-2 text-xs px-4 py-1.5 rounded flex items-center gap-2
              ${loading ? 'bg-gray-400 text-white cursor-not-allowed' : 'bg-black/70 text-white hover:underline cursor-pointer'}`}
            type='button'
        ...
        <button
          disabled={isAdding}
          type='submit'
          className={`mt-8 w-40 h-10 rounded text-sm 
            ${isAdding ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary cursor-pointer text-white'}`}
        >
